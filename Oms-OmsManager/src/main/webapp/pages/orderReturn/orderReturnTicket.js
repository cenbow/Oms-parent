...
var orderSettleBillListUrl = basePath+"/custom/orderSettleBill/orderBillList.spmvc";

var getOrderSettleBillUrl = basePath+"/custom/orderSettleBill/getOrderSettleBill.spmvc";

var mqUrl = basePath+"/custom/orderSettleBill/performActiveMQ.spmvc";

var deleteOrderSettleBillUrl =  basePath+"/custom/orderSettleBill/deleteOrderSettleBill.spmvc";

var orderTicketForm = null;
var orderTicketGrid = null;
var orderTicketJsonStore = null;
...
Ext.onReady(function(){
	...
	orderTicketForm = Ext.widget('form', {
		layout: {
			type: 'vbox',
			align: 'stretch'
		},
	...
	buttons : [
   
	     {
			...
			text : '重置',
			handler : function () {
				resetButton(orderTicketForm);
				billTypeOptionCombo.setValue(3);
			}
		}
		...
		listeners: {
			...
			resize: function(){
				if (collapseResize) {
					setResize(orderTicketForm,orderTicketGrid, null, 5);
				}
			}
		}
	});
	...
	orderTicketJsonStore = createObjectGridStore(orderSettleBillListUrl, pageSize, "MB.OrderSettleBill");
	...
	columns = [
		...
		{ id : 'billTypeStr', header : "业务类型", align : "center", width : 80, dataIndex : 'billTypeStr',sortable:false},
		...
		items: [
			{
				...
				text: '生成调整单',
				handler : function() {
					FormEditWin.showAddDirWin("popWins",basePath+"/custom/orderSettleBill/createOrderSettleBill.spmvc?method=init&orderReturnFlag=true",
							"orderSettlement_winID","创建退单结算 ",900,500);
				}
			} , {
				...
				handler : function() {
					...
					var selModel = orderTicketGrid.getSelectionModel();
	...
	orderTicketGrid = createGridPanelByExt4('orderBillListPageOrderTicketGrid', null, null, orderTicketJsonStore,
	...
	var billNoListener = function (combo, record,index) {
		...
		FormEditWin.showAddDirWin("popWins",basePath+"/custom/orderSettleBill/createOrderSettleBill.spmvc?method=update&billNo="+billNo+"&channelCode="+channelCode+"&orderReturnFlag=true",
				"orderSettlement_winID","创建退单结算 ",900,500);
	};
	
	orderTicketGrid.addListener('rowdblclick',billNoListener);
	...
	var mainPanel = Ext.widget('panel', {
		...
		resizable: false,
		autoHeight: true,
		items: [orderTicketForm, orderTicketGrid]
	});
	billTypeOptionCombo.setValue(3);
	billTypeOptionCombo.setReadOnly(true);
	
	//查询翻页后带入查询数据
	orderTicketJsonStore.on('beforeload', function (store, options){
		Ext.apply(store.proxy.extraParams, getFormParams(orderTicketForm));
	});
	...
	function search(){
	 ...
	 var initParams = {start : 0, limit : pageSize };
		var searchParams = getFormParams(orderTicketForm, initParams);
		...
		orderTicketGrid.store.currentPage = 1;// 翻页后重新查询 页面重置为1
		orderTicketGrid.store.pageSize = pageSize;
		orderTicketGrid.store.load({params : searchParams});
	
		
	}
	
	function exportRecord() {// 导数据
		var searchParams = getFormParams(orderTicketForm);
		if (orderTicketGrid.store.totalCount > 10000) {
