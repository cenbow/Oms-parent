...
var addOrderLogForm = null;
var addOrderLogGrid = null;
var addOrderLogJsonStore = null;
var columns = null;
...
var returnSettlementTypeOptionCombo = null;
...
Ext.onReady(function(){
	...
	var addOrderLogOrderTypeDataStore = Ext.create('Ext.data.Store', {
		model: 'MB.ComboModel',
		data: SELECT.order_sn_type
	});

	addOrderLogOrderTypeOptionCombo = createComboBoxLocal(addOrderLogOrderTypeDataStore, 'addOrderLogOrderType', 'n','v', "local", "分类类型", 230, 80, "orderType","请选择类型");

	var orderType = $("#orderType").val();
	
	if(null != orderType && "" != orderType){
		addOrderLogOrderTypeOptionCombo.setValue(orderType);
		Ext.getCmp('addOrderLogOrderType').setDisabled(true);
	}
...
/**********************************	*/
	...
	//'1,预付款，2保证金',
	returnSettlementTypeOptionCombo = createComboBoxLocal(returnSettlementTypeDataStore,'orderDepositReturnSettlementType', 'n','v', 'local','退款结算状态', 230, 80, 'returnSettlementType', '请选择');
...
/**************************/	
	addOrderLogForm = Ext.widget('form', {
		layout: {
			type: 'vbox',
			align: 'stretch'
		},
...
//		alias : 'widget.addOrderTicket',
			  ...
			  {//第一行
				...
				items: [  
				    //    returnSettlementTypeOptionCombo,
				    	addOrderLogOrderTypeOptionCombo,
				        { 
							xtype : "textfield",
							labelWidth: 80,
	...
	/*	buttonAlign: 'right',
		listeners: {
			...
			resize: function(){
				if (collapseResize) {
					setResize(addOrderLogForm,addOrderLogGrid, null, 5);
				}
			}
		}*/
	});
	var selModel = Ext.create('Ext.selection.CheckboxModel', {
		...
		/*listeners: {
			selectionchange: function(sm, selections, aa) {
				addOrderLogGrid.down('#confirmButton').setDisabled((selections.length === 0 || batchConfirmAuth));
			}
		}*/
...
//		var billNo	 =  $("#billNo").val();
	...
	addOrderLogJsonStore = createObjectGridStore(orderSettleBillListUrl+"?billNo="+billNo, pageSize, "MB.OrderSettleBill");
		
	//} else {
//		addOrderLogJsonStore = createObjectGridStore("", pageSize, "MB.OrderSettleBill");
//	}

	//addOrderLogJsonStore = createObjectGridStore(orderSettleBillListUrl, pageSize, "MB.OrderSettleBill");
	
	columns = [
	//	{ id : 'billNo', header : "单据批次号", align : "center", width : 835, dataIndex:'billNo', sortable:false}//,
		{ id : 'orderCode', header : "订单号或外部交易号", align : "center", width : 835, dataIndex : 'orderCode', sortable:false}//,
		...
		items: [
			...
			//	disabled: true,
				...
				handler : function() {
					...
					FormEditWin.showAddDirWin("popWins",basePath+"/custom/orderSettleBill/enterExportOrderInfoOrOrderReturnLogPage.spmvc?billNo="+billNo,
	...
	addOrderLogGrid = createGridPanelByExt4('orderSettleBillPageaddOrderLogGrid', null, null, addOrderLogJsonStore,pageSize, null, columns, null,dItems);
	
//	addOrderLogGrid = createGridPanelByExt4('orderSettleBill', null, null, null,pageSize, null, columns, selModel,dItems);
	...
	var mainPanel = Ext.widget('panel', {
		...
		resizable: false,
		autoHeight: true,
		items: [addOrderLogForm, addOrderLogGrid]
	});
	
	//查询翻页后带入查询数据
//	addOrderLogJsonStore.on('beforeload', function (store, options){
		
	/*	var searchParams = getFormParams(addOrderLogForm);
...
/*	function showHiddenCondition(b, c){
	...
	function batchConfirm() {
		//批量确认被选择的行订单
		var selModel = addOrderLogGrid.getSelectionModel();
	...
	/*	var initParams = {start : 0, limit : pageSize };
		var searchParams = getFormParams(addOrderLogForm, initParams);
		var userName = Ext.getCmp("orderInfoPageUserName").getValue();
		var consignee = Ext.getCmp("orderInfoPageConsignee").getValue();
		var shippingId = shippingIdOptionCombo.getValue();
		var country = countryCombo.getValue();
		...
		searchParams.tel = Ext.getCmp("orderInfoPageTel").getValue();
		searchParams.invoiceNo = Ext.getCmp("orderInfoPageInvoiceNo").getValue();
		addOrderLogGrid.store.currentPage = 1;// 翻页后重新查询 页面重置为1
		addOrderLogGrid.store.pageSize = pageSize;
		addOrderLogGrid.store.load({params : searchParams});
		*/
	}
	
	function exportRecord() {// 导数据
		var searchParams = getFormParams(addOrderLogForm);
		if (addOrderLogGrid.store.totalCount > 10000) {
