...
<html>
	...
	Ext.require(['*']);
	var importUrl = basePath+"/custom/orderSettleBill/exportOrderInfoOrOrderReturnLog.spmvc";
	...
	Ext.onReady(function() {
		...
		var orderType = parent.addOrderLogOrderTypeOptionCombo.getValue();
	
		Ext.create('Ext.form.Panel', {
			url : importUrl+"?billNo="+billNo+"&orderType="+orderType,
			...
			items : [ {
				...
				html:'<a id="importTemplateFile" style="color:red" href = "'+ basePath+'custom/downloadTemp.spmvc?path=' +encodeURI( encodeURI('/pages/excelModel/订单退单调整日志.xls'))+'">下载导出模板文件（简体中文）</a>'
			} ],
			buttons : [ {
	...
	//				var shippingId = Ext.getCmp('exportTicketPageShippingId').getValue();  //配送方式
					...
					var channelCode = parent.addOrderLogOrderTypeOptionCombo.getValue(); //退单保证金状态
						...
						form.submit({
							success : function(f, action) {
								...
								//	    parent.addOrderTicketJsonStore.getProxy().url=basePath+"/custom/orderSettleBill/createOrderSettleBill.spmvc?billNo="+billNo;
										parent.addOrderLogJsonStore.getProxy().url=basePath+"/custom/orderSettleBill/createOrderSettleBill.spmvc?billNo="+billNo;
								
										parent.addOrderLogJsonStore.load();
										parent.parent.orderLogJsonStore.getProxy().url= basePath+"/custom/orderSettleBill/orderInfoOrOrderReturnLogList.spmvc";
										parent.parent.orderLogJsonStore.load();
