Ext.define("MB.view.orderDetail.QuestionOrder", {
	extend: "Ext.window.Window",
	alias: "widget.questionOrder",
	...
	initComponent: function () {
		...
		this.items = {
				...
				border: false,
				frame: true,
				url : basePath + '/custom/orderStatus/question',
				fieldDefaults: {
					labelAlign: 'left',
					labelWidth: 90
				},
		...
		var json = {
			success : function(formPanel, action) {
				if (action.result.success == "false") {
					Ext.msgBox.remainMsg('订单设置问题单', action.result.msg, Ext.MessageBox.ERROR);
				} else {
					win.close();
			...
			failure : function(formPanel, action, bb ,aa ) {
				console.dir(action);
				Ext.msgBox.remainMsg('订单设置问题单', action.response.responseText, Ext.MessageBox.ERROR);
			},
			waitMsg : 'Loading...'
		};
		form.submit(json);
